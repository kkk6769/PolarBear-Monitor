...
interface DataPoint {
  time: string; cpu: number; mem: number; swap: number; disk: number; netIn: number; netOut: number;
}
...
export default function ServerDetailChart({ server, history }: Props) {
  ...
  if (chartData.length < 2) return <div className="text-center text-muted-foreground text-sm py-12">收集数据中...</div>;
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <ChartCard title="Swap 使用率 (%)" data={chartData} dataKey="swap" color="#EF4444" domain={[0, 100]} />
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
